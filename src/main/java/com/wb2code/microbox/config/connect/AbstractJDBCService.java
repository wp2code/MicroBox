...
import com.wb2code.microbox.annotation.Query;
import com.wb2code.microbox.annotation.QuerySql;
...
public abstract class AbstractJDBCService<T extends BaseTableInfo> implements JDBCService<T> {
    ...
    protected Connection getConnection() {
        Connection connection = dataSourceTypeEnum().getConnection(jdbcSourceInfo);
        return connection;
    }
...
//            LogUtil.info(">>>>>> drop table if exists {}", tableName);
...
//            LogUtil.error("错误！创建表失败【{}】", e.getMessage());
...
//            LogUtil.error("错误！检查表是否存在失败【{}】", e.getMessage());
            throw new RuntimeException(e.getMessage());
        } finally {
    ...
    protected String createTableSql(String tableName, List<DbTableInfo> dbTableInfos) {
        ...
        for (int i = 0; i < dbTableInfos.size(); i++) {
            ...
            if (StrUtil.isNotBlank(dbTableInfo.getComment())) {
                sb.append(" COMMENT ");
                sb.append("'");
...
//        LogUtil.debug("创建表SQL：{}", sb);
    ...
    public boolean insert(T obj) {
        ...
        try {
            connection = getConnection();
            Class<? extends BaseTableInfo> aClass = obj.getClass();
            ...
            if (StrUtil.isNotBlank(tbAnnotation.schema())) {
                sql.append(tbAnnotation.schema());
                sql.append(".");
            }
...
//            LogUtil.info("INSERT SQL:{}", sql);
            if (valueAllIsNull) {
//                LogUtil.error("新增数据失败！{} 字段值都是null", tbAnnotation.value());
                return false;
            }
...
//                LogUtil.debug("数据新增返回主键：{}", pk);
                field.set(obj, pk);
            }
            return true;
        } catch (Exception e) {
//            LogUtil.error("错误！数据新增失败【{}】", e.getMessage());
            throw new RuntimeException(e.getMessage());
        } finally {
            close(connection, statement, resultSet);
        }
    ...
    public boolean updateByPk(T obj) {
        ...
        try {
            connection = getConnection();
            final Class<? extends BaseTableInfo> cls = obj.getClass();
            final Table annotation = cls.getAnnotation(Table.class);
            final String tableName = annotation.value();
            final Field[] declaredFields = ReflectUtil.getFields(cls);
            final Field field = getPrimaryKeyField(declaredFields);
            ...
            sb.append("UPDATE ");
            if (StrUtil.isNotBlank(annotation.schema())) {
                sb.append(annotation.schema());
                sb.append(".");
            }
            sb.append(tableName);
            sb.append(" ");
            ...
            LogUtil.info("UPDATE SQL:{}", sb.toString());
            statement = connection.createStatement();
            statement.executeUpdate(sb.toString());
            return true;
        } catch (Exception e) {
            LogUtil.error("更新数据失败！{}", e.getMessage());
    ...
    public boolean deleteByPk(Long primaryKey, Class<T> cls) {
        ...
        try {
            ...
            final Field field = getPrimaryKeyField(ReflectUtil.getFields(cls));
            if (field == null || primaryKey == null) {
                return false;
            }
            sb.append("DELETE FROM ");
            if (StrUtil.isNotBlank(tbAnnotation.schema())) {
                sb.append(tbAnnotation.schema());
                sb.append(".");
            }
            ...
            LogUtil.error("错误！更新数据失败【{}】】【{}】", primaryKey, e.getMessage());
            throw new RuntimeException(e.getMessage());
        } finally {
            close(connection, statement, null);
        }
    ...
    public int selectCount(T obj) {
        ...
        try {
            ...
            statement = connection.createStatement();
            final Class<? extends BaseTableInfo> cls = obj.getClass();
            Field[] declaredFields = ReflectUtil.getFields(cls);
            List<Field> fields = new ArrayList<>();
            final Table tableAnnotation = cls.getAnnotation(Table.class);
            final String table = tableAnnotation.value();
            StringBuilder sql = new StringBuilder();
            sql.append("SELECT COUNT(*) FROM ");
            if (StrUtil.isNotBlank(tableAnnotation.schema())) {
                sql.append(tableAnnotation.schema());
                sql.append(".");
            }
            sql.append(table);
            ...
            for (Field declaredField : declaredFields) {
                ...
                if (val != null) {
                    ...
                    final Query query = declaredField.getAnnotation(Query.class);
                    if (query != null && query.like()) {
                        if (StrUtil.isNotBlank(likeWhere)) {
                            likeWhere.append(" " + query.accord() + " ");
                        }
                        likeWhere.append(value);
                        likeWhere.append(" like ");
                        likeWhere.append("'%");
                        likeWhere.append(val);
                        likeWhere.append("%'");
                    } else {
                        where.append(" AND ");
                        where.append(value);
            ...
            if (StrUtil.isNotBlank(where)) {
                ...
                if (StrUtil.isNotBlank(likeWhere)) {
                    where.append(" AND ");
                    where.append("(");
                    where.append(likeWhere);
                    where.append(")");
                }
                sql.append(where);
            }
            final QuerySql querySql = cls.getAnnotation(QuerySql.class);
            if (querySql != null) {
                if (StrUtil.isNotBlank(querySql.sql())) {
            ...
            LogUtil.error("错误！查询数据数量失败【{}】", e.getMessage());
            throw new RuntimeException(e.getMessage());
        } finally {
            close(connection, statement, resultSet);
        }
    ...
    public List<T> select(T obj) {
        ...
        try {
            ...
            LogUtil.info("SELECT RESULT:{}", JSONUtil.toJsonStr(list));
            return list;
        } catch (Exception e) {
            LogUtil.error("错误！查询数据数量失败【{}】", e.getMessage());
            throw new RuntimeException(e);
